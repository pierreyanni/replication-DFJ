...
N_z         = 4;    % number of grid points medical expenses shock permanent
...
% pass data to workers to increase speed
                
C_ubar      = parallel.pool.Constant(c_ubar);
R           = parallel.pool.Constant(r);
V_x         = parallel.pool.Constant(v_x);
Inc         = parallel.pool.Constant(inc);
Brackets    = parallel.pool.Constant(brackets);
Tax         = parallel.pool.Constant(tax);
D           = parallel.pool.Constant(d);
Lower_x     = parallel.pool.Constant(lower_x); 
Nu          = parallel.pool.Constant(nu);
Beta        = parallel.pool.Constant(beta);
S           = parallel.pool.Constant(s);
    ...
    parfor n_x = 1:N_x
        ...
        for n_h = 1:N_h
            ...
            if n_x == 1 % when cons floor is reached
                ...
                V_s = objective(c_ubar, r, V_x.Value(n_x), Inc.Value(t+1), ...
                      brackets, tax, med, c_ubar, prob_tr(ind,:), V_x.Value, ... 
                      m_V_f(:, ind), d, lower_x, nu, beta, S.Value(t, n_h));
                cons = c_ubar;
            else
