...
N_grid      = 10; % number of points on grid
...
N_z         = 3;    % number of grid points medical expenses shock permanent
N_eps       = 5;    % number of grid points medical expenses shock transitory
...
m_V_f       = zeros(N_h * N_med, N_grid);               % future value
m_V         = zeros(N_h * N_med, N_grid, N_periods);    % value function
m_c         = zeros(N_h * N_med, N_grid, N_periods);    % consumption choice
m_x_prime   = zeros(N_h * N_med, N_grid, N_periods);    % end-of-period cash-in-hand
year_prob_h = zeros(N_periods, N_h);                    % prob of good health (for graph)
m_med       = zeros(N_periods, N_h * N_med);            % med expenses (graph)
...
for period = N_periods:-1:1
    ...
    med = exp(kron(Xb_med(period,:), ones(N_med, 1)) + kron(Xb_var_med(period,:), v_med));
    % dim (N_med, N_h)
    ...
    for n_h = 1:N_h
        ...
        for n_med = 1:N_med
            ...
            net_cash    = v_x + y - interp1(brackets, tax, y) - med(n_med, n_h);
