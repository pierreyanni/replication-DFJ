...
for t = T-1:-1:1
    ...
    parfor n_z = 1:N_z
        ...
        for n_h = 1:N_h
            ...
            v_cons = zeros(N_x * N_h, 1);
            v_x_p = zeros(N_x * N_h, 1);
            v_V = zeros(N_x * N_h, 1);
            ...
            for n_x = 1:N_x
                ...
                if n_x == 1 % when cons floor is reached
                        ...
                        V_s = objective(c_ubar, r, v_x(n_x), inc(t+1), ...
                            ...
                            m_V_f(N_x*N_h, ind), d, lower_x, nu, beta, s(t, n_h))
                        cons = c_ubar;
                else
                ...
                v_cons((n_h-1) * N_h + n_x, 1)      = cons;
                v_x_p((n_h-1) * N_h + n_x,1)        = v_x(n_x) - cons;
                v_V((n_h-1) * N_h + n_x,1)          = V_s;                 
            end            
        end
