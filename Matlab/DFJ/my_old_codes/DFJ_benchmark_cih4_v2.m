...
m_V         = zeros(N_h * N_x, N_z, T);     % value function
m_c         = zeros(N_h * N_x, N_z, T);     % consumption choice
m_x_p       = zeros(N_h * N_x, N_z, T);     % end-of-period cash-in-hand
...
m_c(:,:,T)      = repmat(v_x, N_h, N_z);
m_V(:,:,T)      = repmat(utility(v_x), N_h, N_z);
...
for t = T-1:-1:1
    ...
    m_V_f       = [m_V(1:N_x, :, t+1) m_V(N_x+1:2*N_x, :, t+1)]; 
                % parallel computation does not work with m_V
    ...
    parfor n_z = 1:N_z
        ...
        for n_h = 1:N_h
            ...
            for n_x = 1:N_x
                ...
                if n_x == 1 % cons floor is reached
                ...
                f = @(c) objective(c, r, v_x(n_x), inc(t+1), ...
                         brackets, tax, med, c_ubar, prob_tr((n_h-1) * N_h + n_z,:), v_x, ... 
                         m_V_f(:, (n_h-1) * N_h + n_z), d, lower_x, nu, beta, s(t, n_h));
...
m_c2       = [m_c(1:N_x, :, t+1) m_c(N_x+1:2*N_x, :, t+1)]

figure(1)
plot(v_x, m_c2(:, :, 30), v_x, v_x, '--', v_x, c_ubar*ones(N_x,1), '--')
title('consumption')
xlabel('cash-in-hand')
